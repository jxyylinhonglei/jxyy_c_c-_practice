...
//		}
//		vector<int> tmp;
//		for (int i = 0; i < p.size(); i++)//找到正则表达式的位置
//		{
//			if (tmp[i] == '*' || tmp[i] == '.')
//			{
//				tmp.push_back(i);
...
//		if (tmp.size() == 0)
//		{
//			return myMatch(s, p);//如果没有正则表达式就比较两个字符串是否一样
//		}
//		//int k =0;
...
//            {
//                max=max>(height[i] > height[j] ? height[j] : height[i])* (j - i)? max: (height[i] > height[j] ? height[j] : height[i]) * (j - i);
//            }
//        }
//        return max;
//    }
...
// //   int arr[] = { 1,2,1 };
// ///*   int arr[] = {1,1};*/
// //   vector<int> tt(arr,arr+ (sizeof(arr) / sizeof(arr[0])));
// //   Solution s;
// //   cout << s.maxArea(tt) << endl;
//    int a = 130 & 128;
//    cout <<a<< endl;
//	return 0;
//}
...
public:
    string intToRoman(int num)
    {
        ...
        string ans;
        for (int i=0;i<sizeof(symba)/sizeof(symba[0]);i++)
        {
            while (num-value[i]>=0)
            {
                ans += symba[i];
    ...
    //    return ans;
    //}
    /* map<char, int>::iterator iter = uset.begin();*/
    int romanToInt(string s)
    {
        map<char, int> uset;
        uset.insert(pair<char,int>('M',1000));
        uset.insert(pair<char, int>('D',500));
        uset.insert(pair<char, int>('C', 100));
        uset.insert(pair<char, int>('L', 50));
        uset.insert(pair<char, int>('X', 10));
        uset.insert(pair<char, int>('V', 5));
        uset.insert(pair<char, int>('I', 1));
        int ans = 0;
        for (int i=0;i<s.size();i++)
        {
            if (uset[s[i]]< uset[s[i+1]]&&i+1< s.size())
            {
                ans -= uset[s[i]];
            }
            else
        ...
        //    int sum = 0;
        //    bool flat = true;
        //    for (int i = 0; i < strs[0].size(); i++)//第一个元素的长度
        //    {
        //        if (!flat)
        //        {
        //            break;
        ...
        //        for (int j = 1; j < strs.size(); j++)//比较的个数
        //        {
        //            if (compare != strs[j][i])
        //            {
        //                flat = false;
        ...
        //        sum = flat ? sum + 1 : sum;
        //    }
        //    return sum == 0 ? "" : strs[0].substr(0, sum);
        //}
        ...
        //   return ret;
        //}
        //int test(int i,int j,vector<vector<int>>& ans)
        //{
        //    int sum = 0;
        ...
        //    return sum;
        //}
        //bool compare(int i,vector<vector<int>>& ans)
        //{
        //        bool flat = true;
        ...
        //                            if (nums[k] <= nums[j])
        //                            {
        //                                vector<int> tmp = { nums[k],nums[j],nums[i] };
        //                                ans.push_back(tmp);
        //                            }
        //                            else
        //                            {
        //                                if (nums[k] <= nums[i])
        //                                {
        //                                    vector<int> tmp = { nums[j],nums[k],nums[i] };
        //                                    ans.push_back(tmp);
        //                                }
        //                                else
        //                                {
        //                                    vector<int> tmp = { nums[j],nums[i],nums[k] };
        //                                    ans.push_back(tmp);
        //                                }
        //                            }
        //                        }
...
vector<vector<int>> threeSum(vector<int>& nums) {
    ...
    return ans;
           ...
           //                return (nums[]);
           //            }
           //            else if (((2*k-right==left+1)||(2 * k - right == left)))//1,4,7,9   6  
           //            {//在两个数中间
           //                ans = { k - 1,k };
           //                break;
           //            }
           ...
           //        }
           //        else if (ans[0]== nums.size() - 1)
           //        {
           //            return (nums[nums.size() - 3] + nums[nums.size() - 2] + nums[nums.size() - 1]);
           //        }
           //        else
           //        {
           //            return nums[ans[0] - 1] + nums[ans[0]] + nums[ans[0] + 1];
           //        }
           //    }
           //    else
           //    {
           //        if (ans[0] == -1)
           //        {
           //            return (nums[ans[1]] + nums[ans[1] + 1] + nums[ans[1] + 2]);
           //        }
           //        else
           //        {
           //            return (nums[ans[0] - 1] + nums[ans[0]] + nums[ans[1]]);
           //        }
           //    }
           // }
};
class solution
{//(+-2020,+-2020),(0 4040,+-2020)
public:
    int test()
    {
        int ans = 0;
        for (int x = -2020; x<=4040 ; x++)//x
        {
            for (int y = -2020; y<= 4020; y++)//y
            {
   ...
   // void merge(int* nums,int*tmp,int left,int right,int mid)
   // {
   //     if (left >= right)
   //     {
   //         return;
   ...
   //     else
   //     {
   //         merge(nums, tmp, left, mid,(left+mid)>>1);
   //         merge(nums, tmp, mid+1,right, (mid+1+right) >> 1);
   //         int begin1 = left;
   //         int end1 = mid;
   //         int begin2 = mid+1;
   //         int end2 = right;
   ...
   //     merge(nums, tmp, left,right, (left+right) >> 1);
   //     delete tmp;
   // }
    void merge(int* nums,int* tmp,int left, int right)
    {
        if (left >= right)
        {
            return;
        ...
        {
            int mid = (left + right) >> 1;//4,2,5  //0,1,2
  ...
  /* void quickSort(int* arr,int begin,int end)
   ...
   void quickSort(int* arr,int begin,int end)
   {
       ...
       else
       {
           ...
           while (left<right)
           {
               while (right > left && tmp <= arr[right])//右边找小
               {
                   right--;
               }
               arr[left] = arr[right];
               while (left < right && arr[left] <= tmp)//左边找大的
               {
                   left++;
               }
               arr[right] = arr[left];
       ...
       while (end<=right)
       {
           if (arr[end] < arr[end-1])
           {
               ...
               while (i>0)
               {
                   if (arr[i] < arr[i - 1])
                   {
                       Swap(&arr[i],&arr[i-1]);//5, 4, 2 //4 5 2//4 2 5
                       i--;
                   }
                   else
                   {
   ...
   void adjustdown(int* arr,int root,int sz)//左右子树都要是小堆或大堆
   {
       //找出左右子树最小或最大
       int parent = root;
       ...
       while (lchild<=sz-1)
       {
           if (lchild+1<=sz-1)
           {
               ...
               {
                   if (arr[lchild + 1] < arr[parent])
                   {
                       Swap(&arr[lchild+1], &arr[parent]);
                       parent = lchild + 1;
                       lchild = 2 * parent + 1;
                   }
                   else
   ...
   void shellSort(int* arr, int sz)
   {
       ...
       while (gap>1)
       {
           ...
           while (end<sz)
           {
               if (end + gap <=sz-1&&arr[end]>arr[end+gap])//前比后大
               {
                   int i = end;
                   while (i>=0)
                   {
...
int main()
{
   ...
   // vector<int> ss = {1,-1,-1,0};
    ...
    //    cout << endl;
    //}
    //int arr[] = { 27,3,2,-10,2,14,13,2.-9,666,32,-1,0,5,9,4,7,-10,11,2,72,-5,-14 };
    //int arr[] = { 49,38,65,97,76,13,27,-10,11,2,72,-5,-14,-3,-3,3,2,-10,9,-15,2,14,-3,-15,-97960,-26521,-61011,83390,-38677,12333,75987,46091,83794,19355 }; //{49,38,65,97,76,13,27,49};//11,2,72,-5,-14,-3,-3,3,2,-10,9,-15,2,14,-3,-15 
    //s1.mergeSort(tet,0, 100000000 -1);
   //int arr[] ={ -10,11,2,24,-11,-3,72,-5,-14,23,5,7,2,44,54,-41,0,66,22,6,1,-55,-8,1,77,-3,-3,3,2,-10,9,-15,2,14,-3,-15 ,58,45,33,77, };//,2,4,1,-5,0,72,-5,-14,23,5,7,21,-55
    int arr[] = { -14,2,66,22,-11,-3,72,-5,-14,23 ,9,38,65,97,76,13,27,-10,11,2,72,-5,-14,-3,-3,3,2,-10,9,-15,2,14,-3,-15,-97960,-26521,-61011,83390,-38677,12333,75987,46091,83794,19355 };
    int sz = sizeof(arr) / sizeof(arr[0]);
    start = GetTickCount();
   ///* s1.insertSort(tet,0, 100000000 -1);*/
    s1.heapSort(arr,sz);
   // // //s1.mergeSort(arr,0,sz-1);
   // //s1.quickSort(arr, 0, sz - 1);
     for (int i = 0; i <sz; i++)
     {
           cout <<arr[i] << " ";
     }
   
    //s1.shellSort(tet, 100000);//int sz = sizeof(arr) / sizeof(arr[0]);
    //s1.quickSort(tet,0, 100000000 - 1);
    //for (int i = 0; i < 100000; i++)
    //{
    //    cout <<tet[i] << " ";
    //}
    //int arr[] = { -10,11,2,72,-5,-14,23,5,7,21,-55,-8,1,77,-3,-3,3,2,-10,9,-15,2,14,-3,-15 ,58,45,33,77,};
    //int sz = sizeof(arr) / sizeof(arr[0]);
    //s1.quickSort(arr, 0, sz - 1);
    //for (int i = 0; i < sz; i++)
    //{
    //      cout <<arr[i] << " ";
    //}
    //s1.bubblesort(arr1, sizeof(arr1) / sizeof(arr1[0]));
    //vector<int> test = {1,1,-1};
    //cout << s.threeSumClosest(test, 0) << endl;
    //s1.bubblesort(tet, 10000);
